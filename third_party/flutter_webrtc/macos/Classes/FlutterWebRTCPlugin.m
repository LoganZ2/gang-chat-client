...
- (void)initialize:(NSArray*)networkIgnoreMask
    ...
    if (!_peerConnectionFactory) {
        ...
        // gang-chat fork: per-app audio device selection on macOS. The
        // CoreAudio ADM (type 0) is LiveKit's documented macOS selection path
...
#if TARGET_OS_OSX
    // gang-chat fork: the CoreAudio ADM's device list is empty until a peer
    // connection is recording/playing. Once it populates (or hardware changes),
    // re-apply any device the user picked before the list was ready.
    [self gcReapplyDesiredDevices];
#endif
    if (self.eventSink) {
      postEvent( self.eventSink, @{@"event" : @"onDeviceChange"});
    }
